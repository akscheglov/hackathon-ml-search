...
уникальные объекты, названия или темы, найденные в контексте. Будь краток и используй только русский язык, ответом должен быть только заголовок.""",
        combine_template="""\
{context_str}. На основании вышеуказанных заголовков и контекста, \
каково полное название этого документа? Будь краток и используй только русский язык, ответом должен быть только заголовок.""",
    ),
    SummaryExtractor(
        summaries=["self"],
        llm=llm,
...
Кратко изложи основные темы и сущности раздела. Для итогового резюме используй русский язык, ответом должено быть только краткое содержание.""",
    ),
    KeywordExtractor(
        keywords=10,
        llm=llm,
...
В ответе используй только русский язык и выведи только ключевые слова через запятую.""",
    ),
    embed_model,
]
